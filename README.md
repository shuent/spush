...
`connection.protocol` は `ftp`、`ftps`、`sftp` に対応しています。`connection.host`、`connection.user`、`connection.password` は `{ env: NAME }` で環境変数から参照できます。`spush init` は protocol に合わせて `.env.spush.example` を作ります。FTP / FTPS では `FTP_HOST`、`FTP_USER`、`FTP_PASSWORD`、SFTPでは `SFTP_HOST`、`SFTP_USER`、`SFTP_PASSWORD` を使う設定になります。既存のように `host: example.com`、`user: myuser` と文字列で直接書く設定もそのまま使えます。

`remote_dir` はサーバー上の公開先ディレクトリです。FTPログイン直後のディレクトリを公開rootとして使うサーバーでは `remote_dir: /` を指定できます。`/.` や `/foo/./` は正規化され、`..` を含む traversal は拒否されます。
...
include: ["**/*"]
exclude: [".DS_Store", ".spush/**", ".git/**", "node_modules/**", ".env", ".env.*"]
...
```bash
SFTP_HOST=example.com SFTP_USER=myuser SFTP_PASSWORD=secret spush push
```

`.env` 系ファイルを使う場合、`spush` は暗黙には読みません。`spush init` で作られる `.env.spush.example` を `.env.spush` にコピーして値を入れ、コマンドで明示するか、`spush.yaml` に `env_file` を指定します。
